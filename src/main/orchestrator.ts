...
const baseAgentProfiles: Array<{
  ...
  },
  {
    ...
    systemPrompt:
      "Ты Объяснитель — объясняешь простыми словами. Отвечай ТОЛЬКО на заданный вопрос. " +
      "Используй факты из веба. НЕ придумывай события, даты, имена. НЕ уходи в посторонние истории. " +
      "Кратко и ясно."
  }
];
...
export async function askQuestion(
  ...
  const webInstruction = webContext
    ? "\n\n[ВЕБ-ДАННЫЕ] Ты получил данные из поиска. Используй ТОЛЬКО их для фактов. Не выдумывай. Отвечай чётко и по теме."
    : "";
  const focusInstruction =
    "\n\nОтвечай СТРОГО на заданный вопрос. Не уходи в другие темы. Будь кратким и точным.";
